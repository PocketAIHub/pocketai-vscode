...
Drive REST and GraphQL diagnosis through PocketAI tools: `run_command` for `curl` or Python `requests`, and `web_fetch` for vendor docs. Isolate the failing layer before guessing at the fix.
...
- Never hardcode tokens in scripts. Read from env (`os.environ["API_TOKEN"]`) or a local secrets file that is never committed.
...
## PocketAI Tool Patterns

### run_command — for curl, dig, openssl

```python
run_command(command="curl -sI https://api.example.com")
run_command(command="openssl s_client -connect api.example.com:443 -servername api.example.com </dev/null 2>/dev/null | openssl x509 -noout -dates")
```

### run_command — for multi-step Python flows

When debugging spans auth → fetch → paginate → validate, run a small Python script with `run_command`. Keep tokens out of logs:
...
### task — for full CRUD test sweeps
